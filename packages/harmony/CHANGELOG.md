...
- EmptyState now fills the width of its container instead of capping itself at 620px. ([#512](https://github.com/Gigadrive/sdk/pull/512))
...
- Tactile design revamp (Harmony 5.0) ([#475](https://github.com/Gigadrive/sdk/pull/475))
  ...
  **Breaking — components**
  ...
  - Type weight scale reduced system-wide: headings, titles, body and controls
    are normal weight (hierarchy via size and color); `font-medium` is reserved
    for small functional emphasis (labels, badges, chips, tabs, toast titles).
    `font.css` now aliases weight 600 to the Medium faces — Resist Sans has no
    SemiBold cut, so `font-semibold` previously resolved up to full Bold (the
    root cause of chronically heavy text).
...
- Remove the `--text-correction` CSS variable and the `wrapTextNodes()` utility. ([#430](https://github.com/Gigadrive/sdk/pull/430))
...
- Redesign `Badge` with a flatter, more tonal look: drop the glossy gradient overlay, inset ([#430](https://github.com/Gigadrive/sdk/pull/430))
  highlight and drop shadow from the filled variants, give `outline` an explicit border with
  muted text, and add a new `soft` variant for tonal labels such as plan or tier badges.

### Patch Changes

- Fix Resist Sans vertical metrics. Two problems, one fix. ([#430](https://github.com/Gigadrive/sdk/pull/430))
...
- Open-source the Network console's declarative layered navigation into Harmony's ([#314](https://github.com/Gigadrive/sdk/pull/314))
