...
export interface NormalizedConfig {
  ...
  /** Managed infrastructure declarations supplied through `gigadrive.yaml`. */
...
/** Declarative File Storage service provisioned in the deployment environment. */
export interface NormalizedConfigServiceStorage extends NormalizedConfigService {
  type: 'storage';
  /**
   * Buckets sorted by name for deterministic deployment plans.
   *
   * Names are preserved exactly after schema validation. The deployment
   * environment is authoritative, and global CDN/S3 slugs are generated by
   * the File Storage API rather than stored in configuration.
   */
  buckets: NormalizedConfigStorageBucket[];
}

/** Desired state for one environment-scoped File Storage bucket. */
export interface NormalizedConfigStorageBucket {
  /** Immutable environment-scoped API and IaC identifier. */
  name: string;
