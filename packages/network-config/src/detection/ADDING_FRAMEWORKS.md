...
| `commands`             | `string[]`                       | yes      | Build commands. The install command (`npm install`, `pnpm install`, etc.) is auto-prepended — do **not** include it. Use `[]` for frameworks that need no build step. |
| `entrypoint`           | `string`                         | yes      | Path to the server entrypoint (relative to project root).                                                                                                             |
