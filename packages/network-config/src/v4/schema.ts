...
export const schema = {
  ...
  properties: {
    ...
    images: {
      type: ['object', 'null'],
      description: 'Managed image optimization policy for this deployment.',
      ...
      properties: {
        localPatterns: {
          ...
          items: {
            ...
            additionalProperties: false,
            properties: {
              pathname: { type: 'string', minLength: 1, maxLength: 2048 },
              search: { type: 'string', maxLength: 2048 },
            },
          },
        },
        remotePatterns: {
          ...
          items: {
            ...
            properties: {
              ...
              port: { type: 'string', pattern: '^[0-9]{0,5}$' },
              pathname: { type: 'string', minLength: 1, maxLength: 2048 },
              search: { type: 'string', maxLength: 2048 },
            },
          },
        ...
        formats: {
          ...
          maxItems: 4,
          uniqueItems: true,
          items: { type: 'string', enum: ['image/avif', 'image/webp', 'image/jpeg', 'image/png'] },
        },
        minimumCacheTTL: { type: 'integer', minimum: 0, maximum: 31536000 },
        dangerouslyAllowSVG: { type: 'boolean' },
        contentSecurityPolicy: { type: 'string', maxLength: 4096 },
        contentDispositionType: { type: 'string', enum: ['inline', 'attachment'] },
        maximumRedirects: { type: 'integer', minimum: 0, maximum: 10 },
        maximumResponseBody: { type: 'integer', minimum: 1024, maximum: 104857600 },
        variants: {
          type: 'object',
          maxProperties: 100,
          ...
          additionalProperties: {
            ...
            properties: {
              ...
              format: { type: 'string', enum: ['image/avif', 'image/webp', 'image/jpeg', 'image/png'] },
    ...
    services: {
      type: 'object',
      description: 'Managed services to provision for the deployment environment.',
      additionalProperties: false,
      properties: {
        redis: {
          oneOf: [
        ...
        storage: {
          ...
          additionalProperties: false,
          description:
            'File Storage buckets to provision in the deployment environment. The environment and global delivery/S3 slug are assigned by the platform.',
          properties: {
            buckets: {
              type: 'object',
              description:
                'Buckets keyed by immutable environment-scoped name. Use null or an empty object for the default private visibility.',
              propertyNames: {
                minLength: 3,
                maxLength: 63,
                pattern:
                  '^(?![0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$)[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$',
              },
              additionalProperties: {
                oneOf: [
                  { type: 'null' },
                  {
                    ...
                    properties: {
                      visibility: {
                        ...
                        enum: ['public', 'private'],
                        description:
                          'Bucket access policy. Defaults to private when omitted. Public buckets can serve objects through their CDN hostname.',
                      },
                    },
                  },
                ],
